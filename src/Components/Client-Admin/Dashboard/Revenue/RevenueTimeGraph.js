...
export default function RevenueTimeGraph({ startDate, endDate, widgetData,marketPlaceId, brand_id, product_id,manufacturer_name, fulfillment_channel, DateStartDate, DateEndDate  }) {
...
const [comparisonText, setComparisonText] = useState('');
  ...
  const [transformDateTime, setTransformDateTime] = useState([]); // Declare transformDateTime state
...
const CustomTooltip = ({ active, payload, label, currentWiseDate, compareWiseData, CompareGrpah }) => {
    if (!active || !payload || payload.length === 0) {
        return null;
    }

    const formatCustomTooltipDate = (dateObj) => {
        if (!(dateObj instanceof Date)) return '';
        const options = { month: 'short', day: 'numeric', hour: 'numeric', minute: 'numeric', hour12: true };
        return dateObj.toLocaleString('en-US', options);
    };

    console.log('compare time', currentWiseDate, compareWiseData);
    const formattedCurrentDate = currentWiseDate?.[0]?.date ? formatCustomTooltipDate(currentWiseDate[0].date) : '';
    const formattedCompareDate = compareWiseData?.[0]?.date ? formatCustomTooltipDate(compareWiseData[0].date) : '';

    console.log('Formatted Dates:', formattedCurrentDate, formattedCompareDate);
    ...
    let formattedFirstTimestampHeader = '';
    ...
    if (firstTimestamp) {
        ...
        formattedFirstTimestampHeader = safeFormatDate(firstTimestamp);
    }
    ...
    return (
        ...
        >
            ...
            {allKeys.map((key, index) => {
                ...
                const compareValue = compareItem?.value ?? undefined; // Use undefined to explicitly check if compare data exists
                ...
                return (
                    ...
                    >
                        ...
                        >
                            ...
                            {/* Compare Value — Only show if CompareGraph is true AND compareValue is defined */}
                            {CompareGrpah && compareValue !== undefined && (
                                <div style={{ textAlign: 'right', minWidth: '100px' }}>
                                    <Typography
                                        style={{
                                            color: '#888',
...
const fetchRevenue = async () => {
    ...
    try {
        ...
        // Transform Graph Data for current period
        const transformedGraphDataCurrent = Object.entries(data.graph || {})
            .filter(([timestamp]) => !isNaN(new Date(timestamp).getTime()))
        ...
        // Transform Graph Data for compare period
        const transformedCompareGraphData = Object.entries(data?.compare_graph || {})
            .filter(([timestamp]) => !isNaN(new Date(timestamp).getTime()))
            .map(([timestamp, values]) => ({
                timestamp,
                date: new Date(timestamp),
                ...
                compareDateLabel: values?.date_label
            }));

        const mergedGraphDataMap = new Map();

        // Merge current period data
        transformedGraphDataCurrent.forEach(entry => {
            mergedGraphDataMap.set(entry.timestamp, { ...entry });
        });

        // Merge compare period data
        transformedCompareGraphData.forEach(entry => {
            const existing = mergedGraphDataMap.get(entry.timestamp) || { timestamp: entry.timestamp, date: entry.date };
            mergedGraphDataMap.set(entry.timestamp, { ...existing, ...entry });
        });

        // Convert merged map to array and sort by timestamp
        const finalMergedGraphData = Array.from(mergedGraphDataMap.values()).sort((a, b) => a.date.getTime() - b.date.getTime());

        setBindGraph(finalMergedGraphData);
        setTransformDateTime(transformedGraphDataCurrent); // You might want to adjust this based on your needs

        if (finalMergedGraphData && finalMergedGraphData.length > 0) {
            const startDate = new Date(finalMergedGraphData[0].date);
            const endDate = new Date(finalMergedGraphData[finalMergedGraphData.length - 1].date);
            ...
            console.log('Formatted Display Date:', transformedGraphDataCurrent);
        }
        // Set the graph data (current period)
        setCurrentFinalDate(transformedGraphDataCurrent);
...
{visibleMetrics.includes('profit_margin') && (
    ...
    <Tooltip
                content={<CustomTooltip
                    CompareGrpah={CompareGrpah}
                    currentWiseDate={transformDateTime} // Make sure this prop contains relevant date info
                    compareWiseData={transformDateTime} // Adjust if you have separate compare date info
                />}
            />   
